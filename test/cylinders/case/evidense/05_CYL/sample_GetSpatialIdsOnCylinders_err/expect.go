...
func main() {
	...
	// 入力を空間IDに変換
	spatialIDs, _ := shape.GetExtendedSpatialIdsOnPoints(pointList, zoom, zoom)
	xIDs := []int64{}
	yIDs := []int64{}
	zIDs := []int64{}
	...
	for ix := minX; ix <= maxX; ix++ {
		...
		for jy := minY; jy <= maxY; jy++ {
			...
			for kz := minZ; kz <= maxZ; kz++ {
				...
				ExtendedSpatialID := shape.GetSpatialIDOnAxisIDs(ix, jy, kz, zoom, zoom)
	...
	// 空間IDのフォーマットに変換
	ExtendedSpatialIDs, err = shape.ConvertExtendedSpatialIdsToSpatialIds(ExtendedSpatialIDs)
	if err != nil {
		fmt.Println(err)
		return
	}
