package main

import (
	"flag"
	"fmt"
	"os"
	"spatial-id/common/enum"
	"spatial-id/common/object"
	"spatial-id/shape"
	"strconv"
)

func main() {
	var (
		output      = flag.String("o", "/dev/null", "output path")
		pointOutput = flag.String("p", "/dev/null", "point output path")
	)

	// コマンドライン引数解析
	flag.Parse()

	// 始点
	sp, err1 := shape.GetPointOnExtendedSpatialId("10/10/10/10/10", enum.Center)
	if err1 != nil {
		fmt.Println(fmt.Errorf("ボクセル中心算出時にエラー発生: %w", err1))
		os.Exit(1)
	}
	start, _ := object.NewPoint(sp[0].Lon(), sp[0].Lat(), sp[0].Alt())

	// 終点
	ep, err2 := shape.GetPointOnExtendedSpatialId("10/13/15/10/18", enum.Center)
	if err2 != nil {
		fmt.Println(fmt.Errorf("ボクセル中心算出時にエラー発生: %w", err2))
		os.Exit(1)
	}
	end, _ := object.NewPoint(ep[0].Lon()-0.00001, ep[0].Lat()+0.00001, ep[0].Alt()-0.00001)
	// 水平精度
	var hZoom int64 = 10
	// 垂直精度
	var vZoom int64 = 10

	// 入力の座標出力
	pp, _ := os.Create(*pointOutput)
	defer pp.Close()

	startLon := strconv.FormatFloat(start.Lon(), 'f', -1, 64)
	startLat := strconv.FormatFloat(start.Lat(), 'f', -1, 64)
	startAlt := strconv.FormatFloat(start.Alt(), 'f', -1, 64)
	endLon := strconv.FormatFloat(end.Lon(), 'f', -1, 64)
	endLat := strconv.FormatFloat(end.Lat(), 'f', -1, 64)
	endAlt := strconv.FormatFloat(end.Alt(), 'f', -1, 64)
	pp.WriteString(startLon + "\n")
	pp.WriteString(startLat + "\n")
	pp.WriteString(startAlt + "\n")
	pp.WriteString(endLon + "\n")
	pp.WriteString(endLat + "\n")
	pp.WriteString(endAlt + "\n")

	// 空間ID取得
	spatialIDs, err3 := shape.GetExtendedSpatialIdsOnLine(start, end, hZoom, vZoom)
	if err3 != nil {
		fmt.Println(fmt.Errorf("空間ID取得時にエラー発生: %w", err3))
		os.Exit(1)
	}

	// 結果を出力
	fmt.Printf("変換後の空間ID: %s\n", spatialIDs)
	fp, _ := os.Create(*output)
	defer fp.Close()

	for _, spatialID := range spatialIDs {
		fp.WriteString(spatialID + "\n")
	}
}
